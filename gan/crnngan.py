...
flags.DEFINE_boolean("generate_meta", False,
                     "Generate the composer and genre as part of output.")
...
model_layout_flags = [
    'num_layers_g', 'num_layers_d', 'meta_layer_size', 'hidden_size_g',
    'hidden_size_d', 'biscale_slow_layer_ticks', 'multiscale', 'multiscale',
    'disable_feed_previous', 'pace_events', 'unidirectional_d',
    'feature_matching', 'composer'
]
...
class RNNGAN(object):
    ...
    def __init__(self, is_training, num_song_features=None, num_meta_features=None):
        ...
        self._input_songdata = tf.placeholder(
            shape=[batch_size, songlength, num_song_features], dtype=data_type())
        self._input_metadata = tf.placeholder(
            shape=[batch_size, num_meta_features], dtype=data_type())
        #_split = tf.split(self._input_songdata,songlength,1)[0]
        print("self._input_songdata", self._input_songdata,
        ...
        with tf.variable_scope('G') as scope:
            ...
            if is_training and FLAGS.keep_prob < 1:
                ...
                cell = make_rnn_cell(
                    [FLAGS.hidden_size_g] * FLAGS.num_layers_g, dropout_keep_prob=FLAGS.keep_prob)
            else:
                cell = make_rnn_cell([FLAGS.hidden_size_g]
                                     * FLAGS.num_layers_g)
            ...
            if FLAGS.generate_meta:
                metainputs = tf.random_uniform(shape=[batch_size, int(
                    FLAGS.random_input_scale * num_meta_features)], minval=0.0, maxval=1.0)
                meta_g = tf.nn.relu(linear(
                    metainputs, FLAGS.meta_layer_size, scope='meta_layer', reuse_scope=False))
                meta_softmax_w = tf.get_variable(
                    "meta_softmax_w", [FLAGS.meta_layer_size, num_meta_features])
                meta_softmax_b = tf.get_variable(
                    "meta_softmax_b", [num_meta_features])
                meta_logits = tf.nn.xw_plus_b(
                    meta_g, meta_softmax_w, meta_softmax_b)
                meta_probs = tf.nn.softmax(meta_logits)

            random_rnninputs = tf.random_uniform(shape=[batch_size, songlength, int(
                FLAGS.random_input_scale * num_song_features)], minval=0.0, maxval=1.0, dtype=data_type())

            # Make list of tensors. One per step in recurrence.
            # Each tensor is batchsize*numfeatures.

            random_rnninputs = [tf.squeeze(input_, [1]) for input_ in tf.split(
                random_rnninputs, songlength, 1)]
            ...
            for i, input_ in enumerate(random_rnninputs):
                ...
                generated_point = linear(
                    output, num_song_features, scope='output_layer', reuse_scope=(i != 0))
        ...
        self._lr = tf.Variable(FLAGS.learning_rate,
                               trainable=False, dtype=data_type())
        self.g_params = [v for v in tf.trainable_variables()
        ...
        self.rnn_pretraining_loss = tf.reduce_mean(tf.squared_difference(x=tf.transpose(
            tf.stack(self._generated_features_pretraining), perm=[1, 0, 2]), y=self._input_songdata))
        if not FLAGS.disable_l2_regularizer:
        ...
        with tf.variable_scope('D') as scope:
            ...
            # TODO: (possibly temporarily) disabling meta info
            print('self._input_songdata shape {}'.format(
                self._input_songdata.get_shape()))
            print('generated data shape {}'.format(
                self._generated_features[0].get_shape()))

            # TODO: (possibly temporarily) disabling meta info
            if FLAGS.generate_meta:
                songdata_inputs = [tf.concat(
                    [self._input_metadata, songdata_input], 1) for songdata_input in songdata_inputs]

            self.real_d, self.real_d_features = self.discriminator(
                songdata_inputs, is_training, msg='real')
            scope.reuse_variables()
            # TODO: (possibly temporarily) disabling meta info
            if FLAGS.generate_meta:
                generated_data = [tf.concat(
                    [meta_probs, songdata_input], 1) for songdata_input in self._generated_features]
            else:
                generated_data = self._generated_features
            if songdata_inputs[0].get_shape() != generated_data[0].get_shape():
                print('songdata_inputs shape {} != generated data shape {}'.format(
                    songdata_inputs[0].get_shape(), generated_data[0].get_shape()))
        ...
        self._new_lr = tf.placeholder(
            shape=[], name="new_learning_rate", dtype=data_type())
    ...
    def discriminator(self, inputs, is_training, msg=''):
        ...
        if is_training and FLAGS.keep_prob < 1:
            inputs = [tf.nn.dropout(input_, FLAGS.keep_prob)
        ...
        if is_training and FLAGS.keep_prob < 1:
            ...
            cell_fw = make_rnn_cell(
                [FLAGS.hidden_size_d] * FLAGS.num_layers_d, dropout_keep_prob=FLAGS.keep_prob)
        ...
        if not FLAGS.unidirectional_d:
            ...
            outputs, state = tf.nn.rnn(
                cell_fw, inputs, initial_state=self._initial_state_fw)

        # decision = tf.sigmoid(linear(outputs[-1], 1, 'decision'))
        if FLAGS.end_classification:
            decisions = [tf.sigmoid(linear(output, 1, 'decision', reuse_scope=(
                i != 0))) for i, output in enumerate([outputs[0], outputs[-1]])]
            decisions = tf.stack(decisions)
            decisions = tf.transpose(decisions, perm=[1, 0, 2])
            print('shape, decisions: {}'.format(decisions.get_shape()))
        else:
            decisions = [tf.sigmoid(linear(output, 1, 'decision', reuse_scope=(
                i != 0))) for i, output in enumerate(outputs)]
            decisions = tf.stack(decisions)
            decisions = tf.transpose(decisions, perm=[1, 0, 2])
            print('shape, decisions: {}'.format(decisions.get_shape()))
...
def run_epoch(session, model, loader, datasetlabel, eval_op_g, eval_op_d, pretraining=False, verbose=False, pretraining_d=False):
    ...
    [batch_meta, batch_song] = loader.get_batch(
        model.batch_size, model.songlength, part=datasetlabel)
    ...
    while batch_meta is not None and batch_song is not None:
        ...
        if pretraining:
            if pretraining_d:
                fetches = [model.rnn_pretraining_loss,
                           model.d_loss, op_g, op_d]
            else:
        ...
        if verbose and iters % 10 == 9:
            ...
            avg_time_in_graph = float(
                sum(times_in_graph)) / float(len(times_in_graph))
            avg_time_in_python = float(
                sum(times_in_python)) / float(len(times_in_python))
            if pretraining:
                print(
                    "{}: {} (pretraining) batch loss: G: {:.3f}, avg loss: G: {:.3f}, speed: {:.1f} songs/s, avg in graph: {:.1f}, avg in python: {:.1f}.".format(
                        datasetlabel,
                        iters,
                        g_loss,
                ...
                print(
                    "{}: {} batch loss: G: {:.3f}, D: {:.3f}, avg loss: G: {:.3f}, D: {:.3f} speed: {:.1f} songs/s, avg in graph: {:.1f}, avg in python: {:.1f}.".format(
                        datasetlabel,
                        iters,
                        g_loss,
                        d_loss,
...
def main(_):
    ...
    songfeatures_filename = os.path.join(
        FLAGS.traindir, 'num_song_features.pkl')
    metafeatures_filename = os.path.join(
        FLAGS.traindir, 'num_meta_features.pkl')

    loader = DataLoader(FLAGS.datadir)
    ...
    if global_step < FLAGS.pretraining_epochs:
        FLAGS.songlength = int(
            min(((global_step + 10) / 10) * 10, songlength_ceiling))
    ...
    with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(log_device_placement=FLAGS.log_device_placement)) as session:
        with tf.variable_scope("model", reuse=None) as scope:
            scope.set_regularizer(
                tf.contrib.layers.l2_regularizer(scale=FLAGS.reg_scale))
            m = RNNGAN(is_training=True, num_song_features=num_song_features,
        ...
        if not FLAGS.sample:
            ...
            for i in range(global_step, FLAGS.max_epoch):
                lr_decay = FLAGS.lr_decay ** max(i -
                                                 FLAGS.epochs_before_decay, 0.0)
                ...
                if new_songlength != FLAGS.songlength:
                    print('Changing songlength, now training on {} events from songs.'.format(
                        new_songlength))
                ...
                if i < FLAGS.pretraining_epochs:
                    ...
                    if FLAGS.pretraining_d:
                        opt_d = m.opt_d
                    train_g_loss, train_d_loss = run_epoch(session, m, loader, 'train', m.opt_pretraining, opt_d,
                                                           pretraining=True, verbose=True, pretraining_d=FLAGS.pretraining_d)
                    if FLAGS.pretraining_d:
                        try:
                ...
                # PALMER: write song data to output file
                ...
                loader.save_midi_pattern(filename, song_data)
